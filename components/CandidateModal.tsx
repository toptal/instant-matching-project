...
export default function CandidateModal({ candidates, currentIndex, decisions, onClose, onDecide, onNavigate }: Props) {
  ...
  function selectReason(reason: string) {
    void reason; // stored for future use
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: 860, maxHeight: "90vh", boxShadow: "0 24px 64px rgba(0,0,0,0.24)" }}
      >
        {/* Close */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-10 w-8 h-8 rounded-full flex items-center justify-center bg-white"
        ...
        {/* Header — dark navy */}
        <div className="relative flex shrink-0" style={{ background: "#1B2D72", minHeight: 240, padding: "32px 32px 24px 32px" }}>
          <div className="flex flex-col gap-4 flex-1 pr-8">
            <div className="flex items-center gap-3">
              <h2 className="text-white font-bold text-[22px] leading-[28px]">{c.name}</h2>
              <div
                className="px-2.5 py-0.5 rounded-lg text-[12px] font-semibold leading-[18px] text-white"
          ...
          {/* Photo card */}
          <div className="shrink-0 bg-white rounded-xl overflow-hidden flex flex-col" style={{ width: 200, boxShadow: "0 4px 16px rgba(0,0,0,0.15)" }}>
        ...
        {/* Bottom action bar */}
        <div className="shrink-0 relative px-6 py-4" style={{ borderTop: "1px solid #EBECED" }}>
